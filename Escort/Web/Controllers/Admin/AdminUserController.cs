...
namespace Web.Controllers.Admin
{
    ...
    public class AdminUserController : AdminBaseController
    {
        ...
        public async Task<IActionResult> AddUsers(int? id)
        {
            ...
            {
                var userDetails = (await _manageService.GetUserDetails(id.Value))?.Data; // Use null-conditional operator
                if (userDetails == null || userDetails.UserId <= 0) // Check for null before dereferencing
                {
        ...
        public async Task<IActionResult> AddUpdateUser(AdminUserRequestModel requestModel)
        {
            ...
            await Task.Factory.StartNew(async () =>
            {
                ...
                {
                    // Fetch permissions and ensure they are not null before passing to SetUserPermissions
                    var permisison = await _manageService.GetUserPermission(requestModel.Id);
                    if (permisison?.Data != null) // Check for null
                    {
